...
const fetchApi = async (url, expectedContentType = 'application/json') => {
    ...
    try {
        ...
        return expectedContentType === 'application/json' ? await response.json() : {
...
const updateStatistics = async (chartStatistics, isFaculty = false) => {
    ...
    try {
        const url = `/api/v1/statistics/${semesterCode}/points/?faculty_id=${facultyID}` + (isFaculty ? '' : `&class_id=${classID}`);
...
const exportFile = async (typeFile, isFaculty = false) => {
    ...
    url += isFaculty ? `&faculty_id=${facultyID}` : `&faculty_id=${facultyID}&class_id=${classID}`;

    let expectedContentType = typeFile === 'pdf' ? 'application/pdf' : 'text/csv';
...
window.onload = () => {
    ...
    let chartStatistics = generateChart(ctxChartStatistics, 'bar', labelsOfFaculty || labelsOfClass,
